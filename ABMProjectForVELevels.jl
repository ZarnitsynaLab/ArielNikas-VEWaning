##--This code is meant to run an example simulation of an influenza epidemic where vaccination is spread over 30 days. The vaccine protection wanes from 100% protection to 0% protection over 60 days post vaccination.
##--Please note that 1: If you loaded in the Project/Manifest.toml files to set your enviroment, comment out the using Package lines
##-- 2: For each simulation 2 files will be output, the first based on the individual and the second based on the timing, you need both to complete all the analysis
##--3: Files will be output into your working directory. You can either: a)search for the filename Example-30Day to find them or b)change work_dir to your desired location
using DataFrames
using Random # for setting the Random seed only
using CSV
using Distributions  # necessary for generating exponential distribution sample
using Distributed

## -- Name the Files You're Going to Output
function namedfile1(df,namefile)
  work_dir="Example-30Day"
